...
hwdata["OneValues"]=1
hwdata["Test_Site"] = hwdata.loc[:, 'home_computer']
hwdata["Test_Site"] = hwdata.loc[:, 'home_computer'].replace("Yes", "At Home")
hwdata["Test_Site"] = hwdata.loc[:, 'Test_Site'].replace("No", "On site")

genderlist = hwdata.loc[:, 'gender'].tolist()
onevaluelist =hwdata.loc[:, 'OneValues'].tolist()
testsitelist =hwdata.loc[:, 'home_computer'].tolist()
testsitelist2 =hwdata.loc[:, 'Test_Site'].tolist()
...
hwdata.loc[:, ('rt_total', "sum_score", "gender", "age", "Test_Site")]

st.header("Descriptive Statistics")

st.subheader("Figure1. Sample Characteristics")
...
st.subheader("Figure2. Test Results")
...
fig5=px.histogram(hwdata, x="sum_score", color="Test_Site")
...
fig10=px.scatter(hwdata.sort_values('age'), x="rt_total", y= "sum_score", trendline="ols", animation_frame="age", range_x=(0,2500), range_y=(0,25))
